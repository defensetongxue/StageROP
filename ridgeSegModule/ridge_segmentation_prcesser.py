...
#             mask=F.interpolate(mask[None,None,:,:],scale_factor=2,mode='nearest').squeeze()
#             if save_path:
#                 torch.save(mask,save_path)
#             if self.mode=='point':
#                 maxvals_list, preds_list=k_max_values_and_indices(
#                     mask.numpy(),self.point_number,self.point_dis)
...
class ridge_segmentation_processer():
    ...
    def __call__(self,img_path,class_,save_path=None):
        with torch.no_grad():
            img=Image.open(img_path)
